...
package com.livrexpress.barcode.qrcode.decoder;
...
final class DecodedBitStreamParser
{
    ...
    static DecoderResult decode(byte[] bytes, Version version, ErrorCorrectionLevel ecLevel, Map<DecodeHintType, ?> hints) throws FormatException
    {
        ...
        do
        {
            ...
            if (mode != Mode.TERMINATOR)
            {
                ...
                {
                    // not really supported; all we do is ignore it Read next 8 bits (symbol sequence #) and 8 bits (parity data), then continue
                    bits.readBits(16);
                }
    ...
    private static void decodeKanjiSegment(BitSource bits, StringBuilder result, int count) throws FormatException
    {
        ...
        // Each character will require 2 bytes. Read the characters as 2-byte pairs and decode as Shift_JIS afterwards
        byte[] buffer = new byte[2 * count];
        int offset = 0;
        while (count > 0)
        {
    ...
    private static void decodeByteSegment(BitSource bits, StringBuilder result, int count, CharacterSetECI currentCharacterSetECI, Collection<byte[]> byteSegments, Map<DecodeHintType, ?> hints) throws FormatException
    {
        ...
        if (currentCharacterSetECI == null)
        {
            // The spec isn't clear on this mode; see section 6.4.5: t does not say which encoding to assuming upon decoding.
            // I have seen ISO-8859-1 used as well as Shift_JIS -- without anything like an ECI designator to give a hint.
